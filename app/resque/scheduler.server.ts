...
export async function nodeScheduler() {
    ...
    scheduler.on('poll', () => {
        logger.info(fileName, 'scheduler polling')
    })
    scheduler.on('leader', () => {
        logger.info(fileName, 'scheduler became leader')
    })
    scheduler.on('error', (error) => {
        logger.info(fileName, `scheduler error >> ${error}`)
    })
    ...
    scheduler.on('workingTimestamp', (timestamp) => {
        logger.info(fileName, `scheduler working timestamp ${timestamp}`)
    })
    ...
    schedule.scheduleJob(everySunday, async () => {
        if (scheduler.leader) {
            logger.info(fileName, '>>> enquing {} job')
