    ...
    const result = await page.evaluate(() => {
        ...
        cards.forEach((card) => {
            ...
            const dateText =
                card
                    .querySelector(
                        '.redblacks-primary-card-content > p:first-of-type'
                    )
                    ?.textContent?.trim() || ''
            ...
            const teamsText =
                card
                    .querySelector(
                        '.redblacks-primary-card-content > p:last-of-type'
                    )
                    ?.textContent?.trim() || ''
            ...
            if (href && href !== '#' && !href.endsWith('#')) {
                ...
                const title = `${h3Text}, ${dateText}, ${teamsText}`
                    .replace(/\s+/g, ' ')
                    .trim()
